...
df = df[['papa', 'nom_Produit', 'LMC30']]

df = df.sort_values('nom_Produit')

#Drop les something else
df = df[~df['papa'].str.contains("something else")]
...
df2 = pd.DataFrame.from_dict(npo_jobs, orient='index', columns=[
    'papa', 'nom_Produit', 'LenStore30', 'lienAchatLenStore'])
...
df2 = df2[['papa', 'nom_Produit', 'LenStore30']]

df2 = df2.sort_values('nom_Produit')

#Drop les something else
df2 = df2[~df2['papa'].str.contains("something else")]
